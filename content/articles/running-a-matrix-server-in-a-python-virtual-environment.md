...
In this article, I outline the setup of a [matrix](https://matrix.org) server in a Python virtual environment on Fedora. This is based on the setup I used for my matrix server [matrix.kplante.com](https://matrix.kplante.com) as of July 2022. Matrix is a decentralized network which bridges clients for instant messaging, video conferencing, and VoIP. It supports [end to end encryption via the Olm and Megolm cryptographic ratchets](https://matrix.org/docs/guides/end-to-end-encryption-implementation-guide/). 
...
We want our matrix server to automatically restart if it crashes for any reason. Normally, I would configure this via systemd, but unfortunately, SELinux was making it very difficult to manage via systemd without running as root. Therefore, I set up a lazy crontab instead. 
